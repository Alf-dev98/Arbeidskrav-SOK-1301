...
df %>%
  ...
  filter(between(tid,2000,2024)) %>%
  ggplot(aes(x = tid,y = verdi, color = var)) +
  geom_line() +
  scale_y_continuous(labels = comma) +
  labs(y = "Verdi (i tusen)") 

#brukte chat gpt til å endre verdiene til tusener av kroner ved hjelp av 
#kommandoen scale
...
figuren over viser utviklingen av BNP i løpende (BNP) og faste priser (MEMOBNP) i Norge mellom perioden 2000 og 2024. Vi kan se at BNP i faste priser har holdt seg mer stabilt, imens BNP i løpende priser har hatt en større økning. Vi kan se at BNP i løpende priser økte betydelig under 2020 til 2022, som kan tyde på at det var en høy prisvekst i denne perioden.

'BNP består av markedsverdien av alle varer og tjenester produsert i et land i løpet av ett år, minus de varene og tjenestene som blir brukt i denne produskjonen.' (forelesningsnotat 5)

BNP i løpende priser viser verdiskapningen målt med dagens priser for hvert år.

BNP i faste priser viser verdiskapningen justert for prisendringer, og brukes når man ønsker å analysere om produksjonen faktisk har økt, uavhengig av inflasjon.
...
koden over lager en ny variabel kalt 'df_wide', som består av en modifisert utgave av df, som rydder opp i datasettet, den adskiller MEMOBNP og BNP til hver sin kollonne og flyttter dem øverst, de har fortsatt de samme verdiene, men datasettet er nå mer ryddig og oversiktlig.
...
df_long <- df_wide %>%
  pivot_longer(
    cols = c(BNP, LBNP, MEMOBNP, LMEMOBNP, DBNP, DMEMOBNP),
    names_to = "var",
    values_to = "verdi"
  )
...
Figuren viser at det enkelte år har vært negativ vekst i reel BNP per person. Store økonomiske kriser fører ofte til at produksjonen går ned, vi kan for eksempel se at det har vært negativ vekst i 2009, som følge av finanskrisen, og i 2020 som følge av korona-pandemien.
